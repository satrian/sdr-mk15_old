...
#define SPI_DMA			1		// Use AT32UC3B DMA controller for transferring data to KSZ8851
//#define DMA_RX		1		// not supported currently
#define PDCA_CHANNEL_1	1
#define PDCA_CHANNEL_2	2
...
extern volatile avr32_pdca_channel_t *pdca_channel_network_tx;
extern volatile avr32_pdca_channel_t *pdca_channel_network_rx;

void ksz8851SendPacketDataNonBlocking(unsigned char *localBuffer, unsigned int length);

/* returns the status of the current SPI DMA operation
*/
static inline uint8_t spi_tx_completed(void) __attribute__ ((always_inline));
static inline uint8_t spi_tx_completed(void)
{
#ifdef SPI_DMA
	return (!(pdca_channel_network_tx->tcr));
#else
	return 1;		// if non-dma mode, show that the operation is complete, since spi_op() and spi_op_nonblocking() were both blocking anyway until the transfer is complete
#endif
}

static inline uint8_t spi_rx_completed(void) __attribute__ ((always_inline));
static inline uint8_t spi_rx_completed(void)
{
#ifdef SPI_DMA
	return (!(pdca_channel_network_rx->tcr));
#else
	return 1;		// if non-dma mode, show that the operation is complete, since spi_op() and spi_op_nonblocking() were both blocking anyway until the transfer is complete
#endif
}
